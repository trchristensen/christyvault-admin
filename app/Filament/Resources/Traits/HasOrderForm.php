        ...
        return [
            Grid::make(12)
                ...
                ->schema([
                                ...
                                Select::make('location_id')
                                    ...
                                    ->afterStateUpdated(function ($state, Set $set, ?Order $record) {
                                        ...
                                        if ($location) {
                                            ...
                                            if (! $record?->trip_id) {
                                                $configurationCode = DeliveryArea::isLocalLocation($location)
                                                    ? VehicleConfiguration::CODE_BOOM_TRUCK
                                                    : VehicleConfiguration::CODE_RACK_TRAILER_FORKLIFT_ONBOARD;
                                                $set(
                                                    'load_preview_vehicle_configuration_id',
                    ...
                    View::make('filament.resources.order-resource.live-load-preview')
                        ->schema([
                            Select::make('load_preview_vehicle_configuration_id')
                                ->label('Preview truck')
                                ->options(fn (): array => VehicleConfiguration::query()
                                    ...
                                    ->pluck('name', 'id')
                                    ->all())
                                ->default(fn (?Order $record): ?int => $record?->trip?->vehicle_configuration_id
                                    ?? app(TripVehicleConfigurationResolver::class)
                                        ->defaultForOrders($record ? [$record] : [])
                                        ?->getKey())
                                ->disabled(fn (?Order $record): bool => filled($record?->trip_id))
                                ->helperText(fn (?Order $record): string => $record?->trip_id
                                    ? 'This order uses the truck selected on its trip.'
                                    : 'Choose a truck to test this standalone order.')
                                ->dehydrated(false)
                                ->live(),
                        ])
                        ->columnSpan([
