<x-filament::section>
    <x-slot name="heading">{{ $day['heading'] }}</x-slot>

    <x-slot name="description">
        {{ $day['date']->format('D, M j') }} · {{ $day['total'] }} {{ \Illuminate\Support\Str::plural('delivery', $day['total']) }}
    </x-slot>
    ...
    @if ($day['grouped_orders']->isEmpty())
        <div class="rounded-lg border border-dashed border-gray-300 px-4 py-8 text-center dark:border-gray-700">
            ...
            />
            <p class="text-sm font-medium text-gray-600 dark:text-gray-300">{{ $day['empty_message'] }}</p>
        </div>
    @else
        <div class="team-deliveries-widget space-y-6">
            @foreach ($day['grouped_orders'] as $plant => $orders)
                <div>
                                    ...
                                    @foreach ($deliveryGroup as $order)
                                        <div class="delivery-order-card {{ $isDeliveryTrip ? 'delivery-trip-stop-card' : 'delivery-order-card-embedded' }}">
                                            <x-delivery-order-summary
                                                ...
                                                :stop-count="$deliveryGroup->count()"
                                                :show-photo-action="$day['is_today']"
                                            />
