...
<div class="p-4 order-container" x-data="{
    ...
    @if ($deliveryPhotos->isNotEmpty())
        <div class="p-4 mt-4 rounded-lg bg-blue-50 dark:bg-blue-950/20">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3" x-data="{ photos: @js($deliveryPhotoViewerItems) }">
                @foreach ($deliveryPhotos as $photo)
                    @php
                        $thumbnailUrl = $deliveryPhotoViewerItems[$loop->index]['url'] ?? $photo->url;
                    @endphp
                    <button type="button"
                        class="block overflow-hidden rounded-lg border border-blue-100 bg-white text-left shadow-sm transition hover:border-blue-300 hover:shadow-md dark:border-blue-900 dark:bg-gray-900"
                        x-on:click="openDeliveryPhotoViewer(photos, {{ $loop->index }})"
                        title="{{ $photo->original_filename ?? 'Delivery photo' }}">
                        <img src="{{ $thumbnailUrl }}" alt="Delivery photo for {{ $record->order_number }}"
